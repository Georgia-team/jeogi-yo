...
로그인은 별도 컨트롤러가 아니라 `JwtAuthenticationFilter`(`UsernamePasswordAuthenticationFilter` 상속)가 `/api/v1/auth/login` 요청을 가로채 처리하고, 이후 모든 요청은 `JwtAuthorizationFilter`가 토큰을 검증해 `SecurityContext`를 채웁니다.
...
| 결제 | 결제 생성, 상세 조회, 목록 검색, 결제 취소, 결제 이력 삭제 | 결제 상태와 주문 상태를 함께 관리하여 상태 불일치를 방지 |
...
| 주문 취소 | `OrderStatus.CANCELLED` 처리 | 취소 가능한 주문 상태인지 검증 후 처리 |
| 결제 취소 | `PaymentStatus.CANCEL` 처리 | 성공 결제만 취소 가능 |
| 취소 연동 | 주문 취소 ↔ 결제 취소 연동 | 한쪽만 취소되어 상태가 어긋나지 않도록 처리 |
| 재고 복구 | 주문/결제 취소 시 상품 재고 복구 | 취소 후 상품 재고 정합성 유지 |
